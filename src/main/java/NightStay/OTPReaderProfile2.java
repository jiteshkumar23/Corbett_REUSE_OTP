...
public class OTPReaderProfile2 extends DataProfile2 {

	static WebElement searchForChatEnterSearch = null;
	static WebDriverWait waitMessageNew = null;

	public static String getMeOTP2(int hourOfClickingOTPButton, int minuteOfClickingOTPButton)
			throws InterruptedException, IOException {

//	public static void main(String[] args) throws InterruptedException, IOException {
		System.out.println("Starting to read OTP from Profile 2");
        EdgeOptions options = new EdgeOptions();
        options.setExperimentalOption("debuggerAddress", "localhost:9090");

		EdgeDriver driver3 = new EdgeDriver(options);
		System.out.println(driver3.getCurrentUrl());
		driver3.manage().timeouts().implicitlyWait(Duration.ofSeconds(60));

		JavascriptExecutor js = (JavascriptExecutor) driver3;

		WebDriverWait wait2 = new WebDriverWait(driver3, Duration.ofSeconds(20));
		
		switchToWhatsappSite(driver3);
		
		try {
			wait2.until(ExpectedConditions
					.visibilityOf(driver3.findElement(By.xpath("//div[@title=\"Search input textbox\"]"))));
		} catch (Exception ab) {
...
//		int minuteOfClickingOTPButton = 11;
		String otp = readOTP(driver3, js, hourOfClickingOTPButton, minuteOfClickingOTPButton);
		System.out.println("Got the MESSAGE: "+otp);
		return otp;
	}

	public static String readOTP(EdgeDriver driver3, JavascriptExecutor js, int hourOfClickingOTPButton,
	    ...
	    waitMessageNew = new WebDriverWait(driver3, Duration.ofSeconds(120));
		waitMessageNew.until(ExpectedConditions
				.numberOfElementsToBeMoreThan((By.xpath("//div[@class='_1BOF7 _2AOIt']")), newmessagelistSize));

		List<WebElement> newmessagelistCheckAgain = driver3.findElements(By.xpath("//div[@class='_1BOF7 _2AOIt']"));
		...
		try {
			...
			if (message.toLowerCase().contains("check your phone")) {
				...
				// Hard Refresh
				((JavascriptExecutor) driver3).executeScript("location.reload(true);");

				searchForChatAndClick(driver3);
				
				List<WebElement> newmessagelistNew2 = driver3.findElements(By.xpath("//div[@class='_1BOF7 _2AOIt']"));
		...
		if (timeOfWhatsAppMessageReceived.isBefore(timeOfClickingOTPSend)) {
			...
			List<WebElement> newmessagelistNewH = driver3.findElements(By.xpath("//div[@class='_1BOF7 _2AOIt']"));
	...
	private static void searchForChatAndClick(EdgeDriver driver3) {
		searchForChatEnterSearch = driver3.findElement(By.xpath("//div[@title=\"Search input textbox\"]"));
		...
		driver3.findElement(By.xpath("//span[contains(@title ,\"85643\") or contains(@title ,\"Zipr\")]")).click();
	...
	private static void switchToWhatsappSite(EdgeDriver driver3) {
		Set<String> windowHandles = driver3.getWindowHandles();

		for (String winHandle : windowHandles) {
			driver3.switchTo().window(winHandle);
			System.out.println(driver3.getCurrentUrl());
			if (driver3.getCurrentUrl().toLowerCase().contains("whatsapp")) {
				break;
			} else {
				continue;
			}
		...
		System.out.println("Current window title is " + driver3.getTitle());
